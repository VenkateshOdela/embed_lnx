...
static int vdrv_probe(struct platform_device *pdev)
{
	...
	if (!dev_node) {
		dev_err(dev, "could not find valid dev_node\n");
		return -EINVAL;
	}
	if (of_property_read_string(dev_node, "virt,dev-serial", &platdata.serial_number)) {
		dev_info(dev, "Missing serial number property\n");
		return -EINVAL;
	}
	if (of_property_read_u32(dev_node, "virt,size", &platdata.size)) {
		dev_info(dev, "Missing size property\n");
		return -EINVAL;
	}
	if (of_property_read_u32(dev_node, "virt,permission", &platdata.permission)) {
		dev_info(dev, "Missing permission property\n");
		return -EINVAL;
	}

	if (of_property_read_u32(dev_node, "virt,dev-id", &val)) {
		dev_info(dev, "Missing permission property\n");
		return -EINVAL;
	}
	...
	devconf->devid = drv_pdata.major_base + minor;
	...
	dev_info(dev, "Probe successful, Device claimed\n");
	dev_info(dev, "--------------------\n");
	return 0;
}
...
struct of_device_id vDev_dt_match[] = {
	{.compatible = "vDev-Ax"},
	{.compatible = "vDev-Bx"},
	{.compatible = "vDev-Cx"},
	{.compatible = "vDev-Dx"},
...
static int __init vdrv_platform_reg(void)
{
	...
	ret =
	    alloc_chrdev_region(&drv_pdata.major_base, 1, NO_OF_DEVS, DRVNAME);
	if (ret < 0)
		goto error_out;

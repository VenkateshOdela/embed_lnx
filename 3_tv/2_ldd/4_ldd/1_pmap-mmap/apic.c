 ...
 *  memory mapped io
 *
 *  #define IOAPIC_BASE 0xFEC00000   // this is physical address , ioremap helps to remap to virtual address
 *
 *    - if we add + offset to this address , will get selection of registers for r/w
 *
 *  - sudo cat /proc/iomem --> we will get above physical address from this cat /proc/iomem
 *
 *
 *   input               ioremap          output
...
#define IOAPIC_BASE 0xFEC00000   // this is physical address , ioremap helps to remap
...
int init_module(void)
{
	...
	/* 
	   ioremap - remaps a physical address range into the processor's virtual address space(kernel's linear address),
	   making it available to the kernel.
